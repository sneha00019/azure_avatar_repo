...
fetch('./js/config.json')
  ...
  var resume = '  KASI KOVID SRI HARSHA \
  Location: Hyderabad, India \
  Contact number: +91 -9346656696  Email id: kovidsriharsha12 @gmail.com \
  Career Objective  A Fresher  who can readily deal with complicated topics. My passion for innovation, \
  research and quick learning abilities have been a driving force in gaining significant \
  knowledge in various domains , which  would help me in achieving the goals of your \
  company. \
  \
  Educational background \
   Pursuing B.tech  in Electr onics  And Communication \
  Engineering  from Sreenidhi Institute of Science and \
  Technology  (201 8-2022) \
  XII from Excellencia Junior College with 90.8 % \
  (201 6-2018) \
  High School (X) from  Reqel Ford International \
  School (CBSE)  with CGPA:  8.4 (201 6) \
  \
  \
  Key skills \
  ...
   Member of IC  square innovative creativity club. \
   Participated in a NSS event \
   Internships  and Certifications \
  Internships: \
   Completed  certification course Machine \
  learning with Python from Verzeo . \
   Completed  certification course on  Introduction \
  to Networks course from Cisco . \
  Workshops : \
   Artificial Intelligence workshop conducted by \
  Kyrion Technologies Pvt.Ltd at IIT Hyderabad. \
  \
  \
  Projects \
   Unique ID based electronic voting system. \
   Sign -talk gesture language translator. \
   Prediction of profits of 50 start -up companies \
  using Machine learning (Multi linear \
  regression) . \
  ...
  Languages: English, Hindi, Telugu , French (beginner). \
  Hobbies: Gaming, Cooking, Playing cricket, Reading \
  Comics , Photoshop and Video Editing.';

console.log(resume);

var job_summary = `Exciting opportunity for a skilled Data Scientist to join our analytics team and drive data-driven decision-making across the organization.
Requirements
...
Responsibility
    ...
    Analyze large datasets to extract valuable insights and trends.
...
function initializeConversation() {
    const systemMessage = [
      {"role": "system", "content": "You are a friendly and interactive AI interviewer for a technology domain job. You will ask the candidate 3 questions, one question at a time, based on the candidate's resume and the job description provided. After receiving the candidate's response, you will ask a follow-up question based on their previous answer. Avoid repeating the same questions. Finally, rate the candidate based on their responses for each question and how well they fit the job role out of 10. Start the interview by asking, 'Hi i am Your AI interviewer. Could you please introduce yourself?"},
